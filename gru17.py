...
print('Build model...')
model = Sequential()
model.add(Conv1D(filters=128,kernel_size=3,padding='same', input_shape=(timesteps,num_features), activation='relu'))
model.add(GRU(128, dropout=0.2, recurrent_dropout=0.2, activation='sigmoid', return_sequences=True ))
model.add(Conv1D(filters=5,kernel_size=3,padding='same', activation='sigmoid'))
