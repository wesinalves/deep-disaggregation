...
print('Loading data...')
with open('../dataset/multivariate/input_signal02.csv', 'rb') as f:
  input_signal = np.loadtxt(f, delimiter='\t')
  f.close()

with open('../dataset/multivariate/output_signal02.csv', 'rb') as f:
...
#model.add(GRU(num_appliances, return_sequences=True, activation='sigmoid' ))
#model.add(GRU(num_appliances, return_sequences=True, activation='sigmoid' ))
