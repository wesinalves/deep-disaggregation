...
print('Loading data...')
with open('../dataset/multivariate/input_signal03.csv', 'rb') as f:
  input_signal = np.loadtxt(f, delimiter='\t')
  f.close()

with open('../dataset/multivariate/output_signal03.csv', 'rb') as f:
...
model.add(LSTM(num_appliances, dropout=0.2, recurrent_dropout=0.2, input_shape=(1,2), return_sequences=True, activation='sigmoid' ))
model.add(LSTM(num_appliances, input_shape=(1,2), return_sequences=True, activation='sigmoid' ))
model.add(LSTM(num_appliances, input_shape=(1,2), return_sequences=True, activation='sigmoid' ))
